      ...
      {isLoading ? (
        ...
        <Card className="bg-slate-800 border-slate-700">
          <CardContent className="pt-6 text-center">
            <p className="text-slate-400 mb-4">No tasks found. Create your first task to get started.</p>
            <Button onClick={() => setShowModal(true)} className="bg-blue-600 hover:bg-blue-700 text-white">
              Create Task
            </Button>
          </CardContent>
        ...
        <div className="space-y-3">
          {filteredTasks.map((task) => (
            <TaskCard key={task._id} task={task} onUpdate={handleTaskUpdated} onDelete={handleTaskDeleted} />
          ))}
        </div>
      )}

      {showModal && <CreateTaskModal onClose={() => setShowModal(false)} onSuccess={handleTaskCreated} />}
    </div>
  )
}
